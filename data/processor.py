...
class InputExampleWiki80(object):
    ...
    def __init__(self, guid, sentence, span1, span2, ner1, ner2, type1, type2, label):
        self.guid = guid
    ...
    def _create_examples(self, dataset, set_type):
        ...
        for example in dataset:
            ...
            examples.append(InputExampleWiki80(guid=None,
                            ...
                            ner1=None,
                            ner2=None,
                            type1=example['h']['type'],
                            type2=example['t']['type'],
...
def convert_examples_to_features_normal(examples, max_seq_length, tokenizer):
    ...
    print('#features', len(features))
    return features
from .utils import get_ent_type
def convert_examples_to_features(examples, max_seq_length, tokenizer, args, rel2id):
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            if use_gpt:
                ...
                if args.use_template_words:
                    prompt = f"[sub] {SUBJECT} [sub] {tokenizer.mask_token} [obj] {OBJECT} [obj] ."#使用的提示模板
                else:
            ...
            #实体类型
            st_ids = tokenizer(str(example.type1), add_special_tokens=False)["input_ids"]
            ot_ids = tokenizer(str(example.type2), add_special_tokens=False)["input_ids"]
            # st_emb = torch.mean(word_embeddings.weight[st_ids], dim=0)
            # ot_emb = torch.mean(word_embeddings.weight[ot_ids], dim=0)
            # so_type = [st_emb,ot_emb]
            # so_type = st_ids + ot_ids
            if len(st_ids)==1:
                st_ids.append(1)
            if len(ot_ids)==1:
                ot_ids.append(1)
            ...
            if ex_index == 0:
                logger.info(SUBJECT+":"+example.type1)
                logger.info(OBJECT+":"+example.type2)
            ...
            num_tokens += sum(inputs['attention_mask'])#使用attention_mask计算输入序列长度，如果过长就用下面的函数进行切割
    ...
    so_type = torch.tensor([o['so_type'] for o in instances])
    ...
    if use_gpt:
        ...
        dataset = TensorDataset(input_ids, attention_mask, token_type_ids, labels, so,so_type)
    else:
        dataset = TensorDataset(input_ids, attention_mask, labels, so, so_type)
